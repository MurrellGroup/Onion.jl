...
function (block::TransformerBlock)(x, positions; rope=identity, mask=0)
    h = x + block.attention(block.attention_norm(x), positions, rope; mask)
    out = h + block.feed_forward(block.ffn_norm(h))
    return out
end
(block::TransformerBlock)(x, positions, rope=identity, mask=0) = block(x, positions; rope, mask)

function (block::AdaTransformerBlock)(x, positions, cond; rope=identity, mask=0)
    h = x + block.attention(block.attention_norm(x, cond), positions, rope; mask)
    out = h + block.feed_forward(block.ffn_norm(h, cond))
    return out
end
(block::AdaTransformerBlock)(x, positions, cond, rope=identity, mask=0) = block(x, positions, cond; rope, mask)
