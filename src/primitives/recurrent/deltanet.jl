...
function deltanet_recurrent(b::Backend,
    ...
    _deltanet_recurrent(b, q, k, v, beta, gate, state)
end
...
function deltanet_recurrent(b::Backend,
    ...
    output, state = _deltanet_recurrent(b, q, k, v, beta, gate, state)
    output = reshape(output, einops"... 1 -> ...")
    return output, state
end

# Primitive implementation
function _deltanet_recurrent(::DefaultBackend,
