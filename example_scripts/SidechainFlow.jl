...
using BSON, Flowfusion, Flux, RandomFeatureMaps, ForwardBackward, Optimisers, Plots, CannotWaitForTheseOptimisers, Onion, LearningSchedules, Serialization, JLD2, StatsBase, Einops
...
function batchrecs(recs; maxatoms = 256, longest_res = 50)
    ...
    for i in 1:length(recs)
        ...
        xyz = recs[i].atom_xyz[:, startpos:startpos + maxatoms - 1]
...
function atomplot(xyz, fixed)
    ...
    plot!(xyz[1,fixed], xyz[2,fixed], xyz[3,fixed], legend = :none, color = :blue, alpha = 0.5, linewidth = 1)
    pl
end
...
function SidechainerV1(dim, depth, shifter_depth, heads)
    layers = (;
        ...
        pos_encoding = Chain(RandomFourierFeatures(3 => 256, 0.5f0), Dense(256 => dim)),
...
BSON.@load "/home/murrellb/MOGMOG.jl/pdb500_allatom.bson" allatom_dataset;
...
scheduler = burnin_learning_schedule(0.0001f0, 0.005f0, 1.1f0, 0.9999f0)
...
for i in 1:iters
    ...
    if i % 10000 == 0
        let model = model |> cpu
            @save "sidechainflow_checkpoint_batch$(i).jld2" model
        end
    end
    if i % 1000 == 0
        gen_b = batchrecs(allatom_dataset[sample(legal_inds, 1, replace = false)]; maxatoms = 128)
        ...
        gif(anim, "/home/murrellb/MOGMOG.jl/quicktrained_$(i).mp4", fps = 15)
    end
end
